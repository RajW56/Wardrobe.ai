...
def get_match_strength(a, b):
    ...
    if b["family"] in COMPLEMENTS.get(a["family"], []):
        score += WEIGHTS["complementary"]

    # FIX-BUG-A: depth contrast only for true Light ↔ Deep opposites.
    # Using != previously also fired for Light/Medium and Medium/Deep,
    # inflating scores for Medium colors against Deep wardrobes.
...
def generate_recommendations(input_data):
    ...
    for name, data in colors.items():
        ...
        for item in wardrobe:
            if item not in colors:
                continue
            match        = get_match_strength(data, colors[item])
            base_compat += match
        ...
        neutral_penalty = 0.0
        if data["neutral"]:
            neutral_penalty = WEIGHTS["neutral"] * NEUTRAL_PENALTY_RATIO * len(wardrobe)
